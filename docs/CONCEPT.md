...
- Game area can be configured between 10x10m to 100x100m
...
- Game must have a topdown camera for clicking tiles, but close-up for character walking for more tension
- Clicking a tile will make the character walk there. 
- Character has Animator, with a 'Speed' float (0-1) controlling transition between Idle and Walking
- Mine prefab is hidden before player moves to the tile. After player reaches, becomes visible and explodes.
- Player dies in one mine. Game over.
- Animator bool 'Death' = true transition to die animation
